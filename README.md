# 🚀 Account Manager - Open Source

一个基于 Cloudflare Workers + D1 + Pages 的账号管理系统。支持个人号库存管理、家庭组车位管理、售出记录、多用户隔离等功能。

> **⚠️ 注意**: 本项目**不包括**任何直接登录第三方平台账号的自动化逻辑。它仅作为一个库存记录和分发管理工具使用。

## ✨ 特性

- **无服务器架构**: 完全基于 Cloudflare 生态，免费、即时扩缩容。
- **多用户支持**: 用户注册/登录 (SHA-256)，数据隔离。
- **安全认证**: Token 采用 HMAC-SHA256 签名，防止伪造。
- **库存管理**: 导入账号、区分个人号/家庭组。
- **车位管理**: 家庭组 5 车位分配、释放、记录买家信息。
- **售出记录**: 个人号售出标记、收入统计。
- **安全**: 2FA 密钥存储与 (实验性) TOTP 查看。
- **移动端优化**: 完美适配手机屏幕，支持布局自动调整与长文本换行。

## 🛠️ 部署指南

### 前置要求
1. Cloudflare 账号
2. Node.js (v18+)
3. Wrangler CLI (`npm install -g wrangler`)

### 1. 准备代码
克隆或下载本项目。

### 2. 创建数据库
```bash
wrangler d1 create account-manager-db
```
记下输出中的 `database_id`。

### 3. 配置后端
1. 编辑 `worker/wrangler.toml`，将 `database_id` 替换为你刚才创建的 ID。
2. **重要**: 编辑 `worker/src/routes/auth.js`，搜索 `admin123`，将其修改为你自己的强密码。这是预设的管理员密码。

### 4. 初始化数据库
```bash
cd worker
wrangler d1 execute account-manager-db --remote --file=../schema.sql
```

### 5. 设置 JWT 密钥（重要！）
```bash
# 在 worker 目录下
wrangler secret put JWT_SECRET
```
输入一个强随机密钥（建议至少32个字符），例如：`Kj8#mP2$vL9@nQ4!xR7*bT1&cY6%hW3`

> ⚠️ 此密钥用于 Token 签名验证，必须设置。如果不设置，系统会使用不安全的默认值。

### 6. 部署后端 Worker
```bash
# 在 worker 目录下
wrangler deploy
```
部署成功后获取 Worker URL (例如 `https://account-manager-worker.xxx.workers.dev`)。

### 7. 配置前端
编辑 `frontend/config.js`：
```javascript
// 环境开关: true=本地开发, false=远程生产
const USE_LOCAL = false;

// 地址配置
const LOCAL_URL = 'http://localhost:8787';
const REMOTE_URL = 'https://your-worker.xxx.workers.dev'; // 替换为你的 Worker URL
```
**只需修改此文件，切换 `USE_LOCAL` 即可在本地和远程环境间切换。**

### 8. 部署前端 Pages
```bash
cd frontend
wrangler pages deploy . --project-name=account-manager
```
访问获得的 Pages URL 即可使用。

## 🔐 登录与安全

### 默认管理员
系统预设了一个硬编码的管理员账号。如果你在部署前已按照步骤 3 修改了密码，请使用你设置的密码登录。

- 用户名: `admin`
- 密码: `admin123` (默认值，建议在部署前修改)

**修改位置**: `worker/src/routes/auth.js` (也可以直接删除那段硬编码逻辑，改用数据库注册的管理员)

### 注册新用户
访问你部署的网站，在登录页点击"立即注册"即可创建一个新用户 (默认为普通用户权限)。

## 📁 目录结构
```
.
├── frontend/             # 静态前端页面
│   ├── config.js         # ⭐ API配置（只需修改此文件）
│   ├── index.html        # 主面板
│   ├── admin.html        # 管理员后台
│   ├── app.js            # 前端逻辑
│   └── ...
├── worker/               # Cloudflare Worker 后端
│   ├── src/              # 源代码
│   │   ├── index.js      # 入口
│   │   └── routes/       # API 路由
│   └── wrangler.toml     # Worker 配置
├── schema.sql            # 数据库结构
└── README.md             # 本文档
```
